...
function useAutocomplete(props: UseAutocompleteProps) {
    const optionBoxFocusDebounce = useRef<NodeJS.Timeout>();

    const findMatch = useCallback(autocompleteFindMatch, []);
    const onInput = useCallback((autocompleteElementEvent: FormEvent<HTMLTextAreaElement> | FormEvent<HTMLInputElement>) => {
        ...
        const matchRef = findMatch({ text, triggers: props.triggers, currentIndex: 0, props });
    ...
    }, [props.optionsBoxReference.current]);
